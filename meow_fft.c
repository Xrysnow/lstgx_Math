...
)
{
	...
	for (unsigned butterfly = 0; butterfly < count; ++butterfly)
	{
		...
		for (unsigned i = 0; i < radix; ++i)
		{
			...
			// W0 is always 1
			//NOTE: fix msvc internal error
			//Meow_FFT_Complex sum = scratch[0];
			float sum_r = scratch[0].r;
			float sum_j = scratch[0].j;
			...
			for (unsigned j = 1; j < radix; ++j)
			{
				...
				if (reverse)
				{
					sum_r += (in.r * w.r) - (in.j * w.j);
					sum_j += (in.r * w.j) + (in.j * w.r);
				}
				else
				{
					sum_r += (in.r * w.r) + (in.j * w.j);
			...
			//out[index_out] = sum;
			out[index_out].r = sum_r;
			out[index_out].j = sum_j;
		}
	}
...
)
{
	// W[0] is always 1.0f;
	{
		...
		Complex t2 = meow_sub(z0, meow_mulf(t1, 0.5f));
